      ...
      {highlightInfo && (
        ...
        >
          <Group justify="space-between" align="center" mb="xs">
            <Group gap="xs">
              <IconAlertCircle color="var(--mantine-color-violet-7)" size={20} />
              <Text fw={600} size="sm" c="violet.9">
                Çakışan Sınavlar (Kural {highlightInfo.rule})
              </Text>
              <Badge color="violet" size="xs">
                {highlightInfo.exams.length} Sınav Kaydı
              </Badge>
            </Group>
          ...
          <Text size="xs" c="dimmed" mb={8}>
            Çakışmaya dahil olan tüm sınav kayıtları aşağıda alt alta listelenmiştir ve takvim üzerinde vurgulanmıştır:
          </Text>

          <Stack gap={6}>
            {highlightInfo.exams.map((exam, idx) => (
              <Paper key={idx} withBorder p="xs" radius="xs" style={{ background: "#FFFFFF" }}>
                <Group justify="space-between" align="center" wrap="wrap" gap="xs">
                  <Group gap="xs">
                    <Badge color="violet" size="sm" variant="filled">
                      {exam.course.code}
                    </Badge>
                    <Text size="xs" fw={600}>
                  ...
                  <Group gap="md">
                    ...
                    <Text size="xs" c="dimmed">
                      ⏰ Saat: {exam.start_time.slice(0, 5)} ({exam.duration_minutes} dk)
                    </Text>
                    <Text size="xs" c="dimmed">
                      🏫 Derslikler: {exam.classrooms.length > 0 ? exam.classrooms.map((c) => c.room_code).join(", ") : "Atanmadı"}
                    </Text>
                  </Group>
                </Group>
              </Paper>
